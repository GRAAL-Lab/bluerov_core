...
namespace ctljsn {
  ...
  void CheckName(std::string name, std::vector<std::string> names) {
      if (std::find(names.begin(), names.end(), name) == names.end()) { // name not found in names
        std::string listOfOkNames("");
        for (auto n : names) listOfOkNames.append(n + " ");
  ...
  //  std::cerr << tc::yellow << "body[status] = " << body["status"] << tc::none << std::endl;
    ...
    for (auto c : capabilities) capabilitiesJson.push_back(c.ToJson());
    body["capabilities"] = capabilitiesJson;

    auto owned_tasksJson = jsoncons::json::make_array<1>(0);
    for (auto p : owned_tasks) owned_tasksJson.push_back(p.ToJson());
  ...
  namespace task {
    ...
    jsoncons::json TaskAdmin::ToJson() const {
      ...
      if (descriptorIsSet_) body["description"] = taskDescriptor.ToJson();
    ...
    jsoncons::json TaskConstraintsBasic::ToJson() const {
      ...
      if (isPosSet) result["target"] = p.ToJson();
      for (auto k : taskConstraintKeys) {
          auto searchRes = dict.find(k);
          if (searchRes != dict.end()) result[k] = searchRes->second;
      }
      return result;
    }
  ...
  namespace geographic {
    ...
    jsoncons::json Position::ToJson() const {
      jsoncons::json result;
      if (usesData) result = data;
      else result["reference"] = query;
      return result;
    }
    ...
    jsoncons::json GenerateDefinedRegion(std::vector<jsoncons::json> boundariesItems) {
      ...
      for (auto p : boundariesItems) boundaries.push_back(p);
  ...
  bool ResolveRef(jsoncons::json &t, const std::shared_ptr<wm::WorldModel> &wm, jsoncons::json &result, std::string text) {
      if (t.contains("reference")) {
          ...
          if (wm != nullptr) queryResult = json_query(wm->ToJson()["body"].as<jsoncons::json>(), query);
  ...
  jsoncons::json DynamicUpdate::ToJson() const {
    ...
    for (auto p : ops) opsJ.push_back(p);
  ...
  bool EvalDynamicUpdate2(DynamicUpdate &du, std::shared_ptr<JSonable> wm, jsoncons::json &result, std::string text) {
    ...
    for (auto operation : opsVec) {
     ...
     // auto item = jsoncons::jsonpointer::get(wm->ToJson(),path.to_string()); // "foo"
      ...
      if (action == "DELETE") {
        if (locations.size() > 0) jsonpath::remove(result, locations[0]);
      }
      else if (action == "PUT") {
      ...
      else if (action == "UPDATE") { throw std::runtime_error("[DynamicUpdate] Case PUT not yet implemented!"); }
      else if (action == "GET") { throw std::runtime_error("[DynamicUpdate] Case GET not yet implemented!"); }
      else { throw std::runtime_error("[DynamicUpdate] Case " + action + " not yet implemented!"); }
      
      std::cerr << "[DynamicUpdate] OK 2" << std::endl;
      if (ec) std::cerr << tc::redL << "[DynamicUpdate] Err: " << ec.message() << tc::none << std::endl;
      else std::cerr << tc::greenL << "[DynamicUpdate] OK!" << tc::none << std::endl;
      std::cerr << "wm after = " << result << std::endl;
      if (result["body"]["resources"]["spatial_primitives"].contains("positions"))
        std::cerr << tc::yellow << "wm pos after = " <<
          result["body"]["resources"]["spatial_primitives"]["positions"] 
          << tc::none << std::endl;
      else
  ...
  bool EvalDynamicUpdate(DynamicUpdate &du, std::shared_ptr<JSonable> wm, jsoncons::json &result, std::string text) {
    ...
    for (auto operation : opsVec) {
     ...
     // auto item = jsoncons::jsonpointer::get(wm->ToJson(),path.to_string()); // "foo"
      ...
      else if (action == "PUT") { throw std::runtime_error("[DynamicUpdate] Case PUT not yet implemented!"); }
      else if (action == "GET") { throw std::runtime_error("[DynamicUpdate] Case GET not yet implemented!"); }
      else { throw std::runtime_error("[DynamicUpdate] Case " + action + " not yet implemented!"); }
  ...
  jsoncons::json GenerateDynamicUpdateItem(std::string action, std::string ref, jsoncons::json value) {
      ...
      if (action.find("DELETE") == std::string::npos) op["value"] = value;
      return op;
  }
  ...
  namespace wm {
    jsoncons::json Resource::ToJson() const {
      ...
      if (markingsIsSet) resourceJson["markings"] = markings.ToJson();
    ...
    jsoncons::json LabelledSpace::ToJson() const {
      ...
      if (spaceType == SpaceType::POS) regJson["position"] = region.ToJson();
      else if (spaceType == SpaceType::REG) regJson["region"] = region.ToJson();
      else if (spaceType == SpaceType::OBJ) regJson["object"] = region.ToJson();
      return regJson;
    }
    ...
    jsoncons::json DataProducts::ToJson() const {
      ...
      for (auto tsk : relevant_tasks) tasksArr.push_back(tsk.ToJson());
      dataProductsJson["relevant_tasks"] = tasksArr;

      auto productsArr = jsoncons::json::make_array<1>(0);
      for (auto s : products) productsArr.push_back(s);
    ...
    jsoncons::json Labels::ToJson() const {
      ...
      for (auto p : bottom_types) arr.push_back(p);
  ...
  jsoncons::json CapabilityDescriptor::ToJson() const {
    ...
    if (authorities.size() > 0) {
      ...
      for (auto a : authorities) authoritiesJson.push_back(a->ToJson());

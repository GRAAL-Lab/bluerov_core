    ...
    : Node("mission_control_node")
{
    ...
    if (systemStatus_->conf.simCtrlStation) {
        simCtrlStationTimer_ = this->create_wall_timer(
            std::chrono::milliseconds(2000), std::bind(&MissionController::SimulateMissionCmdFromFile, this));
    ...
    std::shared_ptr<auv_core_helper::srv::MissionCommand::Response> response)
{
    ...
    response->res = true;
    int msDelay = static_cast<int>(systemStatus_->conf.delayMissionStart * 1000);
    delayMissionStartTimer_ = this->create_wall_timer(
        std::chrono::milliseconds(msDelay),
        std::bind(&MissionController::StartMission, this));

    RCLCPP_INFO_STREAM_THROTTLE(this->get_logger(), *get_clock(), 1000, "Mission will start in " << systemStatus_->conf.delayMissionStart << " seconds");
}
...
bool MissionController::StartMission()
{
    ...
    if (delayMissionStartTimer_) {
        delayMissionStartTimer_->cancel();
    }
...
{
    if (systemStatus_->State() != MissionCtrlState::ON_A_MISSION) {
        // RCLCPP_WARN_STREAM_THROTTLE(this->get_logger(), *get_clock(), 1000, "Received KCL feedback but mission controller is not ON_A_MISSION, ignoring feedback.");
        return;
    }
...
void MissionController::LoadConfiguration()
{
    ...
    try {
        ...
        ctb::GetParam(confObj, systemStatus_->conf.diveDepthBuoys, "dive_depth_buoys");
        ctb::GetParam(confObj, systemStatus_->conf.diveDepthGate, "dive_depth_gate");
        ctb::GetParam(confObj, systemStatus_->conf.diveDepthManipulationConsole, "dive_depth_manipulation_console");
...
void MissionController::SetUpFSM()
{
    ...
    stateDepth_ = std::make_shared<states::StateDepth>();
    ...
    statesMap_.insert({ states::ID::moveToDepth, stateDepth_ });

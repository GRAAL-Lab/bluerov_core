...
class MarineTrackingROS2 : public rclcpp::Node {
    ...
    rclcpp::Time tsRos_;
    void FiltersCallback(const obstacle_tracking_msg::msg::ObstacleArray::ConstPtr& obstaclesMsg);
    bool SetTime(const obstacle_tracking_msg::msg::ObstacleArray::ConstPtr& obstacles);
    void ReadConfigFile(const std::string& dataPath, libconfig::Config& confObj);

    // Subscribers
    std::shared_ptr<message_filters::Subscriber<obstacle_tracking_msg::msg::ObstacleArray>> filtersSub_;
    rclcpp::Publisher< obstacle_tracking_msg::msg::ObstacleArray>::SharedPtr filtersPub_;
